...
public class MaxHeap<T> implements MaxHeapInterface {
	...
	private void siftDown() {
		...
		while (l < firstFreeIndex) {
			...
			int r = l + 1;
			if (r < firstFreeIndex) {
				if (dictionaryEntries.get(r)
						.compareTo(dictionaryEntries.get(l)) > 0) {
					max++;
				}
			}
	...
	**/
	 ...
	 */
	/**
	 * public void loadDictionary() { FileHandler loader = new FileHandler();
	 * try { List wordList = loader.readFile("dictionary\\esp_to_eng.dat"); for
	 * (int i = 0; i < wordList.size();) { add(new
	 * Entry(String.valueOf(wordList.get(i++)),
	 * String.valueOf(wordList.get(i++)))); } } catch (Exception e) {
	 * 
	 * } }
	 **/
	...
	public void loadDictionary(String filename) {
		...
		try {
			...
			for (int i = 0; i < wordList.size();) {
				add(new Entry(String.valueOf(wordList.get(i++)),
						String.valueOf(wordList.get(i++))));
			}
		} catch (Exception e) {
	...
	**/
	 ...
	 */
	/**
	 * @Override public void clear() { dictionaryEntries = new
	 *           ArrayList<Entry>(); }
	 **/
